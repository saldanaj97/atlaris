...
- [x] AC1 — A single plan-domain read boundary owns shared list/detail/status/
      attempt projection semantics for app and API consumers.
- [x] AC2 — Shared consumers stop importing a mix of `read-service`,
      `read-models`, and `status/read-status` to answer one read concern.
- [x] AC3 — Summary/detail/status/attempt outputs keep their current observable
      behavior unless a documented compatibility adjustment is explicitly
      approved during implementation.
- [x] AC4 — Summary display-status semantics, including the paused/staleness
      overlay, are defined once inside the boundary rather than in app helper
      files.
- [x] AC5 — Query integration remains in `src/lib/db/queries/plans.ts` and
...
- [x] AC6 — Boundary-level tests cover observable read outputs; low-level
...
- [x] AC7 — Module-detail scope explicitly resolved pre-implementation:
      stays outside v1. See `plan.md` Step 0.0 item 2 and Resolved Decisions.
- [x] AC8 — `pnpm check:full` + `pnpm test:changed` (unit + integration) passed with
...
- [x] Load live issue `#310` and verify the current issue body/title against the
      repo.
- [x] Verify the current implementation surface across
...
- [x] **Decision frozen — module detail stays outside v1 boundary.**
...
- [x] **Decision frozen — one-shot migration, no long-lived compat barrel.**
...
- [x] Create `src/features/plans/read-projection/` with a narrow public barrel.
- [x] Add the initial orchestration entrypoints described in `plan.md`
      "Proposed Public Surface".
- [x] Keep DB row bundle types internal to the package or sourced from
      `src/lib/db/types.ts` (`PlanDbClient`); do not leak Drizzle query-builder types.
- [x] Delete `src/features/plans/read-service/` once consumer migration in
      Step 4.0 completes (one-shot migration, no long-lived barrel per
      Step 0.0).

### Step 2.0 — Move Projection Semantics Behind The Boundary

- [x] Re-home detail projection logic behind the new boundary:
      `buildLearningPlanDetail`, `toClientPlanDetail`,
      `buildPlanDetailStatusSnapshot`.
- [x] Re-home attempt projection logic behind the new boundary:
      `toClientGenerationAttempts` and attempt normalization rules.
- [x] Re-home summary/lightweight summary projection logic behind the new
      boundary:
      `buildPlanSummaries`, `buildLightweightPlanSummaries`.
- [x] Keep one canonical generation-read status derivation in the same package
      as the projections that consume it (`read-status.ts`).
- [x] Make old helper modules private, local, or deleted once no external
      consumer needs them (removed `read-models/`, `status/read-status.ts`).

### Step 3.0 — Canonicalize App-Level Status Consumers

- [x] Add a boundary-owned summary display-status selector that accepts
      `summary` plus `referenceDate`.
- [x] Move paused/staleness logic out of
      `src/app/plans/components/plan-utils.ts` and into the boundary selector.
- [x] Update `src/app/dashboard/components/activity-utils.ts` to use the same
      selector for ranking/filtering decisions.
- [x] Keep dashboard copy/date formatting in the app layer; move only shared
      status truth.

### Step 4.0 — Migrate Consumers To One Entry Point

- [x] Update API consumers:
...
- [x] Update app/page consumers:
      `src/app/plans/components/PlansContent.tsx`,
      `src/app/dashboard/components/DashboardContent.tsx`,
      `src/app/plans/[id]/actions.ts`.
- [x] Update app helper consumers:
      `src/app/plans/components/plan-utils.ts`,
      `src/app/dashboard/components/activity-utils.ts`.
- [x] Remove mixed-layer imports so one read concern no longer requires multiple
      plan read modules.

### Step 5.0 — Boundary Tests And Query Coverage

- [x] Add boundary unit coverage for summary, lightweight summary, detail,
...
- [x] Add display-status selector matrix covering active, generating,
      paused<30d, paused≥30d, and failed (canonical) cases in
      `read-projection-display-status.spec.ts`.
- [x] Add API response-shape contract tests for the four plan routes
...
- [x] Keep `tests/integration/db/plans.queries.spec.ts` focused on ownership,
      pagination, and query behavior (updated imports to `read-projection`).
- [x] Keep `tests/integration/db/modules.queries.spec.ts` (module detail stays
      outside the new boundary per Step 0.0).
- [x] Slim or delete helper-first tests only after boundary-level replacements
...
- [x] Run focused boundary/unit coverage for the new package.
- [x] Run query integration coverage for plans — `plans.queries.spec.ts` +
      `plans-read.contract.spec.ts` with Testcontainers (2026-04-23).
- [x] Run `pnpm test:changed` — full green with Docker.
- [x] Run `pnpm check:full`.

### Step 7.0 — Issue Verification & Closure

- [x] Walk each acceptance criterion with direct file/test evidence (evidence
      table below).
- [x] Fill the review evidence table below with concrete paths and commands.
- [x] Comment or otherwise capture the evidence on issue `#310` (posted:
      https://github.com/saldanaj97/atlaris/issues/310#issuecomment-4307424609).
...
- [x] Ownership checks still happen before plan detail/status/attempt data is
      exposed (unchanged query layer; same `getPlan*` + null → 404 paths).
- [x] No new service-role read path is introduced where request-scoped DB access
      is required (routes still use `getDb()`; no new service-role reads).
- [x] No response shape accidentally leaks internal-only fields during
      projection consolidation (code move only; contract spec guards shapes).
- [x] Status normalization still handles unknown classifications/statuses
      defensively (`detail-dto.ts` / `detail-status.ts` moved verbatim).
- [x] `src/features/plans/read-service/` is fully deleted before PR review; no
