...
4. Run the Supabase migration workflow for the target environment (`staging-db-migrations.yaml` from `develop`, `production-db-migrations.yaml` from `main`), which applies committed migrations with `supabase db push --include-all`.
...
1. Run the environment workflow (`staging-db-migrations.yaml` from `develop`, `production-db-migrations.yaml` from `main`). Hosted workflows always use `supabase db push --include-all`.
2. For a manual CLI push outside Actions, use `supabase db push --include-all` against the target project when out-of-order historical versions are present (see `docs/architecture/retention-cleanup-runbook.md` and the email delivery runbook).
