...
**Method Signature:** `toolkit.createProject(projectOptions?: ProjectCreateRequest['project']): Promise<ToolkitProject>`
...
console.log(
  `Project created. Connection URI: ${project.connectionURIs[0].connection_uri}`
);
...
**Method Signature:** `toolkit.sql(project: ToolkitProject, query: string): Promise<any>`
...
await toolkit.sql(
  project,
  `INSERT INTO tasks (description) VALUES ('Analyze user feedback');`
);
...
**Method Signature:** `toolkit.deleteProject(project: ToolkitProject): Promise<void>`
...
async function runTemporaryDatabaseTask() {
  ...
  try {
    ...
    await toolkit.sql(
      project,
      `INSERT INTO logs (message) VALUES ('Task started');`
    );
