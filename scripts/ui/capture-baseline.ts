...
function spawnNextDev(mode: 'anon' | 'auth', stateFilePath: string): ChildProcess {
	const env = {
		...process.env,
...
function validateViewportDimensionsDistinct(entries: CaptureManifestEntry[]): void {
	const viewportOnly = entries.filter((e) => e.variant === 'viewport' && e.status === 'ok');
	const widths = new Set(
		viewportOnly.map((e) => e.width),
	);
...
function validateFullPageWidth(entry: CaptureManifestEntry, vp: { width: number }): void {
...
async function main(): Promise<void> {
	...
	const skipInfraPartial =
		(anonBase !== null) !== (authBase !== null);
	if (skipInfraPartial) {
	...
	try {
			...
			for (const variant of VARIANTS) {
				for (const route of ANON_ROUTES) {
					...
					const entry: CaptureManifestEntry = {
						...
						expectedHeightMin:
							variant === 'viewport' ? vp.height : vp.height,
						width: 0,
						height: 0,
						status: 'ok',
					};
		...
		console.log(`[baseline] Wrote ${manifestEntries.length} entries under ${resolvedOut}`);
		if (failed.length > 0) {
