...
const DAILY_BAR_CHART_MARGIN = { top: 8, right: 8, left: 4, bottom: 8 };
const AREA_CHART_MARGIN = { top: 12, right: 8, left: 4, bottom: 8 };
const COMPLETION_AREA_GRADIENT_ID = 'usage-completion-area-fill';
...
export function DailyActivityBarChart({
  ...
  days: UsageAnalyticsDayRow[];
  metric: 'time' | 'changes';
  labelledBy?: string;
  describedBy?: string;
}) {
  const seriesName = metric === 'time' ? 'Estimated time' : 'Progress changes';
  const chartData = days.map((day) => ({
    day: day.label,
    value:
      metric === 'time'
        ? day.estimatedCompletionAddedMinutes
        : day.progressChangeCount,
  }));
  const maxValue = Math.max(1, ...chartData.map((day) => day.value));

  return (
    <WithRecharts
      fallback={
        <output
          aria-label='Loading learning activity chart'
    ...
    >
      {({
        ...
        <figure
          className='m-0 h-64 w-full overflow-visible'
          aria-describedby={describedBy}
          aria-labelledby={labelledBy}
        >
          <ResponsiveChartContainer
            ResponsiveContainer={ResponsiveContainer}
            aria-hidden='true'
            config={{ value: { label: seriesName, color: 'var(--chart-1)' } }}
            className='aspect-auto h-64 w-full overflow-visible'
          >
            <BarChart
              data={chartData}
              margin={DAILY_BAR_CHART_MARGIN}
              barCategoryGap='58%'
            >
              <CartesianGrid vertical={false} strokeDasharray='4 6' />
              <XAxis
...
export function TimeByPlanDonut({
  shares,
  totalMinutes,
  labelledBy,
  describedBy,
}: {
  shares: UsageAnalyticsPlanTimeShare[];
  totalMinutes: number;
  labelledBy?: string;
  describedBy?: string;
}) {
  ...
  const chartConfig: ChartConfig = Object.fromEntries(
    chartData.map((share) => [
      share.id,
      { label: share.topic, color: share.color },
    ]),
  );
  ...
  return (
    <WithRecharts
      fallback={
        <output
          aria-label='Loading time by plan chart'
    ...
    >
      {({ Cell, Pie, PieChart, ResponsiveContainer, Tooltip }) => (
        <figure
          className='m-0 flex min-h-56 flex-col items-center gap-6 sm:flex-row sm:items-center'
          aria-describedby={describedBy}
          aria-labelledby={labelledBy}
        >
          <div className='relative size-44 shrink-0'>
            <ResponsiveChartContainer
              ...
              aria-hidden='true'
              config={chartConfig}
              className='aspect-auto size-44 overflow-visible'
            >
              <PieChart>
                <Pie
                  data={chartData}
                ...
                >
                  {chartData.map((share) => (
                    <Cell key={share.id} fill={share.color} />
                  ))}
                </Pie>
                <Tooltip
                  cursor={false}
          ...
          <ul className='flex min-w-0 flex-1 flex-col gap-3'>
            {chartData.length > 0 ? (
              chartData.map((share) => (
                ...
                >
                  ...
                  <span className='shrink-0 text-right text-xs text-muted-foreground tabular-nums'>
                    {formatCompactDuration(share.completedMinutes)} (
                    {share.percent}%)
                  </span>
                </li>
              ))
              ...
              <li className='text-sm text-muted-foreground'>
                No estimated completed time yet.
              </li>
            )}
          </ul>
        </figure>
  ...
  const chartData = weeks.map((week, index) => ({
    ...
    value:
      mode === 'cumulative'
        ? weeks
            .slice(0, index + 1)
            .reduce((sum, row) => sum + row.completedEvents, 0)
        : week.completedEvents,
  }));
  const maxValue = Math.max(1, ...chartData.map((row) => row.value));

  return (
    <WithRecharts
      fallback={
        <output
          aria-label='Loading completed events chart'
          className='h-64 w-full'
        />
      }
    >
      {({
        ...
        >
          <ResponsiveChartContainer
            ...
            aria-hidden='true'
            config={{
              value: { label: 'Completed events', color: 'var(--chart-1)' },
            }}
            className='aspect-auto h-64 w-full overflow-visible'
          >
            <AreaChart data={chartData} margin={AREA_CHART_MARGIN}>
              <defs>
                <linearGradient
                  id={COMPLETION_AREA_GRADIENT_ID}
                  x1='0'
                  y1='0'
                  x2='0'
                  y2='1'
              ...
              <Area
                ...
                strokeWidth={2}
                fill={`url(#${COMPLETION_AREA_GRADIENT_ID})`}
              />
            </AreaChart>
          </ResponsiveChartContainer>
        </figure>
