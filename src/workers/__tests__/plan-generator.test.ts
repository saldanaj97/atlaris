...
describe('PlanGenerationWorker (Phase 7)', () => {
  it('processes a plan generation job end-to-end (J026 success + T072 + T073)', async () => {
    ...
    const jobId = await enqueueJob(JOB_TYPES.PLAN_GENERATION, planId, userId, payload);
  ...
  it('processes jobs in priority/creation order under load (J026 concurrency + T071)', async () => {
    ...
    const jobIds = await Promise.all([
      enqueueJob(JOB_TYPES.PLAN_GENERATION, planId, userId, { idx: 'high-a' }, 10),
      enqueueJob(JOB_TYPES.PLAN_GENERATION, planId, userId, { idx: 'high-b' }, 10),
      enqueueJob(JOB_TYPES.PLAN_GENERATION, planId, userId, { idx: 'mid-a' }, 5),
      enqueueJob(JOB_TYPES.PLAN_GENERATION, planId, userId, { idx: 'mid-b' }, 5),
    ...
    vi.spyOn(workerService, 'processPlanGenerationJob').mockImplementation(
      async (job) => {
        order.push({ id: job.id, priority: job.priority, createdAt: job.createdAt });
        return stubResult;
      }
    );
