...
-- Shared setup for pgTAP and Basejump Supabase Test Helpers
-- Runs first to ensure extensions and helpers are available for all tests
-- Install pgTAP and common prerequisites (idempotent)
...
-- Install the Basejump Supabase Test Helpers package and extension
select
  dbdev.install ('basejump-supabase_test_helpers');

create extension if not exists "basejump-supabase_test_helpers";

-- TAP plan for setup completion
select
  plan (1);

select
  ok (true, 'Test setup completed successfully');
