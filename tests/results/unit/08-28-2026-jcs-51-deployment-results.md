...
- Result: passed — 2 test files, 31 tests; Vitest duration 1.75s.
...
- Scope: deployment classification, deploy-aware automatic Staging eligibility, credential-safe remote Preview builds, honest checkout-SHA inspection evidence, per-lane concurrency, outer timeout budgets, exact-SHA CircleCI gates, exact-SHA Staging evidence, cumulative Production expand coverage, immutable migration workflow identity, and CircleCI test-result collection.
