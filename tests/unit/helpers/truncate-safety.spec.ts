...
describe('assertSafeToTruncate (via truncateAll)', () => {
  ...
  it('passes when POSTGRES_URL is not set', async () => {
    delete process.env.POSTGRES_URL;
    await expect(truncateAll()).resolves.not.toThrow();
  });

  it('passes when ALLOW_DB_TRUNCATE is true', async () => {
    process.env.POSTGRES_URL = 'postgres://host/myapp_prod';
    process.env.ALLOW_DB_TRUNCATE = 'true';
    await expect(truncateAll()).resolves.not.toThrow();
  });

  it('passes for a test-suffixed DB name', async () => {
    process.env.POSTGRES_URL = 'postgres://host/postgres_test';
    await expect(truncateAll()).resolves.not.toThrow();
  });

  it('passes for a DB name ending in _tests', async () => {
    process.env.POSTGRES_URL = 'postgres://host/myapp_tests';
    await expect(truncateAll()).resolves.not.toThrow();
  });

  it('passes for worker-isolated test database names', async () => {
    process.env.POSTGRES_URL = 'postgres://host/atlaris_test_w2';
    await expect(truncateAll()).resolves.not.toThrow();
  });

  it('passes for the template and base atlaris test databases', async () => {
    process.env.POSTGRES_URL = 'postgres://host/atlaris_test_template';
    await expect(truncateAll()).resolves.not.toThrow();
    process.env.POSTGRES_URL = 'postgres://host/atlaris_test_base';
    await expect(truncateAll()).resolves.not.toThrow();
    process.env.POSTGRES_URL = 'postgres://host/atlaris_test';
    await expect(truncateAll()).resolves.not.toThrow();
  });

  it('rejects ambiguous middle-of-name test matches that look production-adjacent', async () => {
    process.env.POSTGRES_URL = 'postgres://host/myapp_test_archive';
    await expect(truncateAll()).rejects.toThrow(
      /Refusing to truncate non-test database "myapp_test_archive"/,
    );

    process.env.POSTGRES_URL = 'postgres://host/prod_tests_data';
    await expect(truncateAll()).rejects.toThrow(
  ...
  it('throws for a malformed POSTGRES_URL', async () => {
    process.env.POSTGRES_URL = 'not-a-valid-url';
    await expect(truncateAll()).rejects.toThrow(
      /invalid POSTGRES_URL for safety/,
    );
  });
